Provide candidate variant sites to seed inference windows (the --candidates feature).
  ...
  > ${DORADO_BIN} variant -vv --device cpu ${in_bam} ${in_ref} -t 4 ${model_var} --candidate-filtering --candidate-centered-regions --candidates ${in_candidates} --window-len 300 --window-overlap 100 --variant-flanking-bases 100 --ignore-read-groups -o out 2> out/stderr
  > echo "Exit code: $?"
...
Additionally provide a regions BED file to limit where the candidate sites are considered for inference.
  ...
  > model_var=${MODEL_ROOT_DIR:+--models-directory ${MODEL_ROOT_DIR}}
  > ${DORADO_BIN} variant -vv --device cpu ${in_bam} ${in_ref} -t 4 ${model_var} --regions ${in_candidate_bed} --candidate-filtering --candidate-centered-regions --candidates ${in_candidates} --window-len 300 --window-overlap 100 --variant-flanking-bases 100 --ignore-read-groups -o out 2> out/stderr
  > echo "Exit code: $?"
  > grep "\[error\]" out/stderr | sed -E 's/.*\[/\[/g'
  > grep "\[warning\]" out/stderr | sed -E 's/.*\[/\[/g'
  > ### Remove the qual field because Torch results can vary slightly cross-platform.
...
Tiled region selection is the default windowing approach for candidate-filtered inference.
  ...
  > model_var=${MODEL_ROOT_DIR:+--models-directory ${MODEL_ROOT_DIR}}
  > ${DORADO_BIN} variant -vv --device cpu ${in_bam} ${in_ref} -t 4 ${model_var} --candidate-filtering --candidates ${in_candidates} --window-len 300 --window-overlap 100 --ignore-read-groups -o out 2> out/stderr
  > echo "Exit code: $?"
  > grep "\[error\]" out/stderr | sed -E 's/.*\[/\[/g'
  > grep "\[warning\]" out/stderr | sed -E 's/.*\[/\[/g'
  > ### Remove the qual field because Torch results can vary slightly cross-platform.
...
Candidate filtering with an empty candidates file should succeed and produce no calls.
  ...
  > ${DORADO_BIN} variant -vv --device cpu ${in_bam} ${in_ref} -t 4 ${model_var} --candidate-filtering --candidate-centered-regions --candidates out/empty_candidates.list --window-len 300 --window-overlap 100 --variant-flanking-bases 100 --ignore-read-groups -o out 2> out/stderr
  > echo "Exit code: $?"
  > grep "\[error\]" out/stderr | sed -E 's/.*\[/\[/g'
  > grep "\[warning\]" out/stderr | sed -E 's/.*\[/\[/g'
