...
Zero read groups in the input, and a model is specified by path.
Allowed because `--model-override` allows any model to be used.
  $ rm -rf out; mkdir -p out
  > in_dir=${TEST_DATA_DIR}/polish/test-01-supertiny
  > in_bam="data/in.01.zero_read_groups.bam"
  > model_var="--model-override ${MODEL_DIR}"
  > # Run test.
  > ${DORADO_BIN} polish --device cpu ${in_bam} ${in_dir}/draft.fasta.gz ${model_var} -v > out/out.fasta 2> out/out.fasta.stderr
  > # Eval.
  > echo "Exit code: $?"
...
Two read groups are present in the input BAM, but both belong to the same basecaller model.
