...
std::vector<int64_t> compute_collated_padded_shape(const std::vector<Sample>& buffered_samples) {
    ...
    for (int64_t i = 0; i < std::ssize(buffered_samples); ++i) {
        ...
        for (int64_t j = 0; j < std::ssize(sample.features.sizes()); ++j) {
...
std::vector<int64_t> compute_collated_padded_shape(const std::vector<Sample>& buffered_samples,
    ...
    // Find max on every dimension (mock padding).
    for (int64_t j = 0; j < std::ssize(new_sample.features.sizes()); ++j) {
